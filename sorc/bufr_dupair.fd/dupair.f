...
C   TAMDAR IB SUBTYPE 008.  INFORMATION IS READ SEPARATELY FROM EACH
C   FILE THAT IS PRESENT, AND IS THEN COMBINED INTO TABLES USED FOR THE
C   DUP-CHECK.  VARIOUS OPTIONS ARE EXERCISED IN IDENTIFYING DUPLICATES
C   DEPENDING ON THE COMBINATION OF SUBTYPES BEING COMPARED.  THE
C   WORKING FILE NAMES OF THE ONE, TWO, THREE OR FOUR INPUT DUMP FILES
C   (IN EITHER THE "NEW" FORM x_ttt.sss, WHERE ttt IS BUFR TYPE, sss IS
C   BUFR SUBTYPE, AND x IS AN ORDERING INDEX; OR THE "OLD" FORM
C   ttt.sss) ARE READ FROM STANDARD INPUT (UNIT 5) AT THE START OF THIS
C   PROGRAM. THE OUTPUT DUP-CHECKED FILES WILL BE WRITTEN TO THE SAME
C   FILE NAMES.  ALL OTHER FILE CONNECTIONS ARE MADE THROUGH THE
...
C     FROM MNEMONIC "ICLI" TO "BORG" (FOR BULLETIN BEING MONITORED) IN
C     INPUT 004.001, 004.002 AND 004.003 WORKING DUMP FILES AFTER
C     5/2002 (WILL STILL WORK PROPERLY FOR DUMP FILES PRIOR TO 5/2002)
...
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      CALL W3TAGB('BUFR_DUPAIR',2013,0013,0054,'NP22')

      print *
      print * ,'---> Welcome to BUFR_DUPAIR - Version 01-13-2013'
      print *
...
C  ---------------------------------------------------
      ...
      LOOP1: DO L=1,NTAB-1
         ...
         I = IORD(L)
         J = IORD(L+1)
         DO WHILE(NINT(ABS(TAB_8(2,I)-TAB_8(2,J))*100.).LE.
     .    NINT(DEXY*100.) .AND. JDUP(I).EQ.0)
            DELL = MAX(.05_8,DEXY*MAX(TAB_8(7,I),TAB_8(7,J)))
      IF(NINT(ABS(TAB_8(2,I)-TAB_8(2,J))*100.).LE.NINT(DELL*100.) .AND.
     .   NINT(ABS(TAB_8(3,I)-TAB_8(3,J))*100.).LE.NINT(DELL*100.) .AND.
     .   NINT(ABS(TAB_8(4,I)-TAB_8(4,J))*100.).LE.NINT(DDAY*100.) .AND.
     .   NINT(ABS(TAB_8(5,I)-TAB_8(5,J))*100.).LE.NINT(DRHR*100.) .AND.
     .   NINT(ABS(TAB_8(6,I)-TAB_8(6,J))*100.).LE.NINT(DELV*100.)) THEN
               IF(TAB_8(7,I).EQ.1) JDUP(I) = 1  ! If duplicates, always
               IF(JDUP(I).EQ.0)    JDUP(J) = 1  !  throw out Carswell/
            ENDIF                               !  Tinker/AFWA
